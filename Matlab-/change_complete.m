...
for j = 1:100
    ...
    I = 0;
    e_d = pproc-repmat(receiver(j,:),npoints,1);
    e = e_d.*e_d;
    d = sqrt(sum(e,2));
    [d_er,id]= min(d);
    ...
    for i = 1:npoints
        rad = sqrt((pproc(i,1)-receiver(j,1))^2+(pproc(i,2)-receiver(j,2))^2);
        if rad<0.1 && rad > d_er
            alpha = 2;
...
%Mul = wave_l*3.14*((R_c)^2); %T

%func = @(rxxx) exp((-Mul*I1(TT,rxxx,2,2,4))-(TT*noise*(R_c^2)*rxxx)); %T
%func1 = @(rxx) exp((-Mul*hyp_geo(-2,TT)*rxx)-(TT*noise*(R_c^2)*(rxx^2))); %T 
...
%ls1 = linspace(1,10000,100000); %T
%ls = linspace(0.000001,1,1000); %T
%fs_p1 = []; %T
%fs_p = []; %T
...
%Int_func = sum(fs_p*abs((ls(2)-ls(1)))); %T
%Int_func1 = sum(fs_p1*abs((ls1(2)-ls1(1)))); %T
%Sim_AvgPr = Mul*(Int_func+Int_func1); %T 
