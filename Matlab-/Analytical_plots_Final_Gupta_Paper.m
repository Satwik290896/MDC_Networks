...
%func = @(rxxx) exp((-Mul*I1(TT(i),rxxx,2,2,4)*var(m)));

%func1 = @(rxx) exp((-Mul*hyp_geo(-2,TT(i))*rxx)*var(m));

func = @(rxxx) exp((-Mul*I1(TT(i),rxxx,2,2,4))-(TT(i)*var(m)*(R_c^2)*rxxx));
func1 = @(rxx) exp((-Mul*hyp_geo(-2,TT(i))*rxx)-(TT(i)*var(m)*(R_c^2)*(rxx.^2)));
