...
% This is to simulate 100 different instances of Y-plane
XY_dar = 0.5*rand(100,2);
S = [];

% Emitter coordinate as per the row
em_X_cod = 2;
emitter_X = med_proc(em_X_cod,:);

% emp is used to store P(SINR > Thresh) fo each Thresh value
emp = [];

% Dictionaries for 100 possible scenarios
D = zeros(npoints,n1,100);

% Transmission Power
P_T = 4000;

% Frequency & Wavelength, Noise
freq = 1.5*10^9;
wave_l = (3*10^8)/freq;
%noise = 0.1;
noise = 0.3162;
...
for j = 1:100
    ...
    %Receiver at (0,0) and hece this is the received power
    P_R = P_T * (wave_l/(4*3.14*d_er))^2;
    
    %Reciver to Emitter & other BS' length
    radd = sqrt(sum(med_proc.^2,2));

    % For both n1 BS in X-plane
    for i = 1:n1
        rad = radd(i);
        
        % Intensity received based on the BS to receiver distance in X-plane
        if rad<0.1 && rad~=d_er
            alpha = 2;
    ...
    % SINR for all possible 100 iterations
    SNR = P_R/(I+noise);
    SNR_avg = SNR_avg+ SNR;
end
...
% Model1 SINR extrapolated using dictionary from Y-plane
T = -75
T_end = 30
len = 10000;
...
% Cutoff Radius while calculating Power
R_c = 0.1;
...
% Dictionaries for 100 possible scenarios
D = zeros(npoints,npoints,100);
XPT = repmat(P_T,npoints,1);
Xn = repmat(noise,npoints,1);

% Find Dictionary for 100 instances
%for i=1:100
...
for xxxx = Thre
    ...
    % For 100 different scenarios (XY_dar)
    for j = 1:100
        idd = [];
        d_er = [];
...
X = med_proc';
fileID = fopen('Values_T_Model1_Rayleigh_-5dB.txt','a+');
fprintf(fileID,'%f\n',emp);
fclose(fileID);