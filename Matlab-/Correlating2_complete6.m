...
%Both terms has been taken. Normal distribution has been taken with first
%term as mean and second term as standard deviation. No optimization
%condition
...
n1 = 2;
med_proc = [0.890000 0.980000;0.01 0.001];
...
for i=1:100
    ...
    for j = 1:npoints
        pt = [];
        tmp = repmat(D(j,:,i)',1,2).*med_proc
        for k = 1:n1;
            Temp = sum(repmat(tmp(k,:),n1,1).*tmp,2);
            an = sum(Temp)-Temp(k);
            pt = [pt;an];
        end
        ptt = [ptt;sum(pt)];
...
ddd = zeros(npoints,100);

for i = 1:npoints
    for j = 1:100
        ddd(i,j) = normrnd(S(i,j),abs(P(i,j))*n1/npoints);
    end
end
...
X = med_proc';
fileID = fopen('Values_T_Model3.txt','a+');
